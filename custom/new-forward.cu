...
/*
    ...
    - Using streams to overlap computation with data transfer (4 points)
      (This is not present here since it makes OpTimes higher. It is in the ../op folder.)
    - Total: 12 points
*/
...
*/
__global__ void conv_forward_kernel_tiled_1(float *output, const float *input, const float *mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K, const int W_grid) {
    ...
    int b = blockIdx.z;
    int m = blockIdx.x;
    int h = (blockIdx.y / W_grid)*tile_out_width+threadIdx.y;
    int w = (blockIdx.y % W_grid)*tile_out_width+threadIdx.x;

    // load the whole input tensor into shared memory
    if (h < Height && w < Width)
        tile_3d(0, threadIdx.y, threadIdx.x) = __float2half(in_4d(b, 0, h, w));
    else
        tile_3d(0, threadIdx.y, threadIdx.x) = 0;
    __syncthreads();
    ...
    if (threadIdx.y < tile_out_width && threadIdx.x < tile_out_width) {
        for (int p = 0; p < K; p++) {
            for (int q = 0; q < K; q++) {
                acc += __hmul(tile_3d(0, threadIdx.y+p, threadIdx.x+q), mask_4d(m, 0, p, q));
            }
        }
...
__global__ void conv_forward_kernel_tiled_2(float *output, const float *input, const float *mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K, const int W_grid) {
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    int W_grid = (Width_out-1)/(BLOCK_WIDTH-K+1) + 1;
    if (Channel == 1) {
        conv_forward_kernel_tiled_1<<<DimGrid, DimBlock>>>(device_output, device_input, device_mask, Batch, Map_out, Channel, Height, Width, K, W_grid);
    }
    else {
        conv_forward_kernel_tiled_2<<<DimGrid, DimBlock>>>(device_output, device_input, device_mask, Batch, Map_out, Channel, Height, Width, K, W_grid);
    }
}
